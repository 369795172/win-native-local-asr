...
- **WinLocalASR.Resources** (`net10.0`) — resx string tables (English neutral + `zh-Hans` satellite); shared by App and Tests.
...
| 电平表 (level meter) | RMS level, `(dB+80)/70` clamped to [0,1], fed to HUD | Same formula computed over live capture chunks (IEEE-float RMS variant — metering runs at capture time, not at resample time, so it stays live under the batched stop-time resampler), same HUD consumption | None |
...
| HUD | `DictationHUD` presentation states: recording (red, timer, level), processing (yellow, spinner), copied (green ✓ 1.5 s), cancelled (grey ↯ 1.5 s), error (message, 3 s); clipboard failure renders red ✗ inside error | Implemented (`Core/Hud` + `App/Hud`): borderless topmost `WS_EX_NOACTIVATE \| WS_EX_TOOLWINDOW \| WS_EX_TRANSPARENT \| WS_EX_LAYERED` (no-activate + click-through, never in Alt-Tab), bottom-right of the primary working area via pure `HudPlacement`, PerMonitorV2 DPI scaling, same six-presentation state machine from Core events; recording timer `"mm:ss / mm:ss"` event-driven, processing shows the ticking processing-elapsed timer (Swift `processingDuration(at:)` parity); feedback lifetimes controller-owned | Position anchors bottom-right of the primary screen instead of Mac's bottom-center of the mouse's screen (Windows adaptation, ratified here) |
